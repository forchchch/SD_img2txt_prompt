...
visual_image = Image.open("./training_data/images/dog1.jpg")
...
def clip_train_generation(pipe, i2t_prompt_model, adapter, prompt, file_name, guidance, eta=0.5, preprocess=None, device=None):
    adapter.eval()
    pipe.scheduler = EulerAncestralDiscreteScheduler.from_config(pipe.scheduler.config)
    pipe = pipe.to(device)
    with torch.no_grad():
        reference_image = preprocess(visual_image).unsqueeze(0).cuda()
        ...
        prompt_embeds = pipe._encode_prompt(prompt,device,num_images_per_prompt,
                ...
                negative_prompt_embeds = None)
        img_feature = torch.cat([torch.zeros_like(img_feature),img_feature]) if do_classifier_free_guidance else img_feature
        joint_feature = adapter(img_feature, prompt_embeds)
        tv_prompt = joint_feature + prompt_embeds
        #tv_prompt = torch.cat([prompt_embeds,i2t_emb], dim=1)
...
def clip_evaluation_generation(pipe, i2t_prompt_model, adapter, prompt, reference_image, guidance, eta=0.5):
    i2t_prompt_model.eval()
    pipe.scheduler = EulerAncestralDiscreteScheduler.from_config(pipe.scheduler.config)
    with torch.no_grad():
        ...
        img_feature = torch.cat([torch.zeros_like(img_feature),img_feature]) if do_classifier_free_guidance else img_feature
        joint_feature = adapter(img_feature, prompt_embeds)
        tv_prompt = eta*joint_feature + prompt_embeds
        #tv_prompt = torch.cat([prompt_embeds,i2t_emb], dim=1)
