...
from visualization import prompt_based_generation
...
def main(args):
    ...
    img_downnet = Downnet(img_channel=4)
    ######################obtain the hyperparameter for img2text prompt model#############################
    with torch.no_grad():   
        test_img = torch.randn(1, 3, args.resolution, args.resolution)
        test_latent = vae.encode(test_img).latent_dist.sample()
        test_down_latent = img_downnet(test_latent)
        tdl_size = test_down_latent.size()
        i2t_linear = tdl_size[1]*tdl_size[2]*tdl_size[3]
    
    img2txt_prompt_model = I2T_Prompt(down_net=img_downnet, text_emb_size=1024, linear_dim=i2t_linear)
    ...
    params_to_optimize = (
        [
            ...
            {"params": img2txt_prompt_model.parameters(), "lr": args.learning_rate}
    ...
    if args.train_text_encoder:
        (unet, text_encoder, optimizer, train_dataloader, lr_scheduler,img2txt_prompt_model) = accelerator.prepare(unet, 
                                         text_encoder, optimizer, train_dataloader, lr_scheduler, img2txt_prompt_model)
    else:
        (unet, optimizer, train_dataloader, lr_scheduler,img2txt_prompt_model) = accelerator.prepare(unet, 
                                          optimizer, train_dataloader, lr_scheduler, img2txt_prompt_model)
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if accelerator.sync_gradients:
                params_to_clip = (
                    itertools.chain(unet.parameters(), text_encoder.parameters(), img2txt_prompt_model.parameters()) if args.train_text_encoder
                    else itertools.chain( unet.parameters(), img2txt_prompt_model.parameters())
                )
                ...
                if accelerator.sync_gradients:
                    if args.save_steps and (global_step - last_save)/args.gradient_accumulation_steps >= args.save_steps:
                        if accelerator.is_main_process:
                            ...
                            save_torch_model( accelerator.unwrap_model(img2txt_prompt_model, **extra_args), filename_i2tp )
                            if args.train_text_encoder:
                                save_lora_weight(
                                    pipeline.text_encoder,
                                    filename_text_encoder,
                                    target_replace_module=["CLIPAttention"])
                            prompt_based_generation(pipeline, vae, img2txt_prompt_model, "", os.path.join(oimg_dir, str(epoch)+"_"+str(global_step)+"_"+"recon.jpg"), guidance_scale)
                            prompt_based_generation(pipeline, vae, img2txt_prompt_model, "in the flowers", os.path.join(oimg_dir, str(epoch)+"_"+str(global_step)+"_"+"edit.jpg"), guidance_scale)
